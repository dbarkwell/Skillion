...
namespace Skillion
{
    public class SkillRequestParser : ISkillRequestParser
    {
        ...
        public SkillRequestParser(IWebHostEnvironment webHostEnvironment, IOptions<SkillionConfiguration> skillionConfiguration)
        {
        ...
        public async ValueTask<SkillRequest> ParseHttpRequestAsync(HttpRequest httpRequest)
        {
            ...
            if (string.IsNullOrEmpty(_skillionConfiguration.SkillId) && _skillionConfiguration.AlwaysValidateSkillId)
                throw new Exception("Unable to process request. Always validate skill id is true and skill id is missing");
